...
**SMTP mailer.** The `Mailer` engine wraps PHPMailer, loads SMTP configuration from the settings table, and logs all send attempts to `email_log`. SMTP credentials are never logged — error messages are automatically redacted. The privacy flow calls the Mailer directly: data exports trigger a customer acknowledgment email, deletion requests trigger both a customer confirmation and an operator notification (sent to `notification_email` on the tenant, falling back to the tenant's contact email). Email dispatch is synchronous within the HTTP request (10-second SMTP timeout); the privacy action always succeeds regardless of email outcome, but the HTTP response may be delayed by the SMTP round-trip. If SMTP is unconfigured, no outbound connections are made and emails fail immediately. The `sendTest()` method allows operators to verify their SMTP configuration from the admin panel.
